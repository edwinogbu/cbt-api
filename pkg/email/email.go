...
// SendAdminWelcomeEmail sends welcome email when admin account is created
func (s *EmailService) SendAdminWelcomeEmail(to, firstName, lastName, code string) error {
	if s.templates == nil {
	...
	htmlBody, err := s.renderTemplate("admin_welcome.html", data)
	if err != nil {
		return fmt.Errorf("failed to render admin welcome template: %w", err)
	}
	subject := fmt.Sprintf("Welcome to %s – Admin Account Created", data.SiteName)
	return s.sendAsync(to, subject, htmlBody)
}

// SendAdminVerifyEmail sends OTP verification email
func (s *EmailService) SendAdminVerifyEmail(to, firstName, code string) error {
	if s.templates == nil {
		return fmt.Errorf("email service not initialized – templates not loaded")
	}
	data := TemplateData{
	...
	htmlBody, err := s.renderTemplate("admin_verify.html", data)
	if err != nil {
		return fmt.Errorf("failed to render admin verify template: %w", err)
	}
	subject := fmt.Sprintf("Verify Your Email – %s", data.SiteName)
	return s.sendAsync(to, subject, htmlBody)
}

// SendSchoolRegistrationEmail sends school registration confirmation
func (s *EmailService) SendSchoolRegistrationEmail(to, firstName, schoolName, schoolCode string) error {
	if s.templates == nil {
		return fmt.Errorf("email service not initialized – templates not loaded")
	}
	data := TemplateData{
	...
	htmlBody, err := s.renderTemplate("school_registered.html", data)
	if err != nil {
		return fmt.Errorf("failed to render school registration template: %w", err)
	}
	subject := fmt.Sprintf("School Registered – %s", data.SiteName)
	return s.sendAsync(to, subject, htmlBody)
}

// SendOnboardingCompleteEmail sends onboarding completion email
func (s *EmailService) SendOnboardingCompleteEmail(to, firstName, schoolName, tier, amount string) error {
	if s.templates == nil {
		return fmt.Errorf("email service not initialized – templates not loaded")
	}
	data := TemplateData{
	...
	htmlBody, err := s.renderTemplate("onboarding_complete.html", data)
	if err != nil {
		return fmt.Errorf("failed to render onboarding complete template: %w", err)
	}
	subject := fmt.Sprintf("Onboarding Complete – Welcome to %s", data.SiteName)
	return s.sendAsync(to, subject, htmlBody)
}

// SendPaymentLinkEmail sends payment link for subscription
func (s *EmailService) SendPaymentLinkEmail(to, firstName, tier, amount, paymentLink string) error {
	if s.templates == nil {
		return fmt.Errorf("email service not initialized – templates not loaded")
	}
	data := TemplateData{
	...
	htmlBody, err := s.renderTemplate("payment_link.html", data)
	if err != nil {
		return fmt.Errorf("failed to render payment link template: %w", err)
	}
	subject := fmt.Sprintf("Complete Your Payment – %s Subscription", data.SiteName)
	return s.sendAsync(to, subject, htmlBody)
}

// SendWelcomeEmail - keep for backward compatibility
