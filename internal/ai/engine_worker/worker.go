...
func (w *AIWorker) processGenerateJob(payload map[string]interface{}) {
	...
	if err != nil {
		w.updateJobStatus(jobIDStr, "failed", err.Error())
		return
	}
	...
	for i, q := range questions {
		if results[i].Status == "approved" || results[i].Status == "needs_refinement" {
			optsStorage := w.convertEngineOptionsToStorage(q.Options)
			...
			if err := w.qRepo.Create(ctx, &question); err != nil {
	...
	w.updateJobStatus(jobIDStr, "completed", msg)
}
...
func (w *AIWorker) updateJobStatus(jobIDStr string, status, message string) {
...
func (w *AIWorker) convertEngineOptionsToStorage(opts []engine.Option) models.OptionStorage {
	if len(opts) == 0 {
...
// 	for i, q := range questions {
// 		if results[i].Status == "approved" || results[i].Status == "needs_refinement" {
// 			// FIXED: Use proper OptionStorage converter
// 			optsStorage := w.convertEngineOptionsToStorage(q.Options)
...
// // FIXED: Convert engine options to OptionStorage type
// func (w *AIWorker) convertEngineOptionsToStorage(opts []engine.Option) models.OptionStorage {
// 	if len(opts) == 0 {
// 		return models.OptionStorage{}
// 	}
